...
function CPlatform() {
    ...
    for (var i in sheets) {
      ...
      for (var r in rules) {
        if (a.is(rules[r].selectorText)) {
          o = $.extend(
            o,
            my_platform.css2json(rules[r].style),
            my_platform.css2json(a.attr("style"))
          );
        }
      }
    }
  ...
  this.input_numeric = function (uiElement, bRemoveDot) {
    if (my_platform.var_check(uiElement)) {
      uiElement
        .off("keydown.input_numeric")
        .on("keydown.input_numeric", function (e) {
  ...
  this.ui_sorting = function (
    ...
    if (
      my_platform.var_check(uiSortField) &&
      my_platform.var_check(sOrderby) &&
      my_platform.var_check(iTemplateCount) &&
      my_platform.var_check(uiTemplateForSort) &&
      my_platform.var_check(uiTemplateContainer)
    ) {
      ...
      if (iTemplateCount > 1) {
        ...
        uiTemplateForSort.sort(function (a, b) {
          ...
          if (
            my_platform.var_check(akey) &&
            my_platform.var_check(bkey) &&
            akey.length > 0 &&
            bkey.length > 0
          ) {
        ...
        if (
          returnDescCtr > 0 ||
          returnAscCtr > 0 ||
          uiTemplateForSort.length - 1 > returnZeroCtr
        ) {
          // console.log('not same');
          if (uiTemplateContainer.find(".last-content").length > 0) {
            uiTemplateForSort
              .detach()
              .insertBefore(uiTemplateContainer.find(".last-content"));
          } else {
  ...
  this.readURL = function (file, imgcontainer, filearray) {
    if (
      my_platform.var_check(file) &&
      my_platform.var_check(imgcontainer) &&
      my_platform.var_check(filearray)
    ) {
...
(window.my_platform = new CPlatform()), (window.platform = window.my_platform);
