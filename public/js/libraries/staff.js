(function () {
     ...
     *
     * */
    CPlatform.prototype.staff = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiStaffTable = $('#staff-table');
            uiCreateStaffForm = $('#create-faq');
            uiEditStaffForm = $('#edit-faq');
            uiStaffDatatable = null;
            ...
            uiStaffDatatable = platform.staff.initialize_datatable();

            /* create press validation */
            uiCreateStaffForm.find('[type="submit"]').on('click', function () {
            ...
            uiEditStaffForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            $('body').on('click', '.delete-faq-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-faq-id')},
                            'url': self.attr('data-faq-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                if (platform.var_check(uiStaffDatatable)) {
                                                    uiStaffDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiStaffDatatable = platform.faq.initialize_datatable();

                                                /* check if there are other presses to hide the table header and show the no presses found */
                                                if ($('tr[data-faq-id]').length == 0) {
                                                    $('.faq-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiStaffDatatable)) {
                uiStaffDatatable.destroy();
            }

            /* presses table initialize datatable */
            uiStaffDatatable = uiStaffTable.DataTable({
                "order": [],
                "paging": true,
                "pageLength": 10,
