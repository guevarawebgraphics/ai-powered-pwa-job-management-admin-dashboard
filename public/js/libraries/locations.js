(function () {
     ...
     *
     * */
    CPlatform.prototype.location = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiLocationTable = $('#locations-table');
            uiCreateLocationForm = $('#create-location');
            uiEditLocationForm = $('#edit-location');
            uiLocationDatatable = null;
            ...
            uiLocationDatatable = platform.location.initialize_datatable();

            /* create press validation */
            uiCreateLocationForm.find('[type="submit"]').on('click', function () {
            ...
            uiEditLocationForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            $('body').on('click', '.delete-location-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-location-id')},
                            'url': self.attr('data-location-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                if (platform.var_check(uiLocationDatatable)) {
                                                    uiLocationDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiLocationDatatable = platform.region.initialize_datatable();

                                                /* check if there are other presses to hide the table header and show the no presses found */
                                                if ($('tr[data-location-id]').length == 0) {
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiLocationDatatable)) {
                uiLocationDatatable.destroy();
            }

            /* presses table initialize datatable */
            uiLocationDatatable = uiLocationTable.DataTable({
                "order": [],
                "paging": true,
                "pageLength": 10,
