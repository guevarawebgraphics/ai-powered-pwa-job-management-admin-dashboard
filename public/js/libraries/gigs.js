(function () {
    ...
    /*
     * This js file will only contain gig events
     *
     * */
    CPlatform.prototype.gig = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiGigsTable = $('#gigs-table');
            uiCreateGigForm = $('#create-gig');
            uiEditGigForm = $('#edit-gig');
            uiGigsDatatable = null;
            ...
            uiGigsDatatable = platform.gig.initialize_datatable();

            /* create gig validation */
            uiCreateGigForm.find('[type="submit"]').on('click', function () {
            ...
            /* edit gig validation */
            uiEditGigForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            /* delete gig button ajax */
            $('body').on('click', '.delete-gig-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-gig-id')},
                            'url': self.attr('data-gig-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove gig container */
                                                // $('[data-gig-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiGigsDatatable)) {
                                                    uiGigsDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiGigsDatatable = platform.gig.initialize_datatable();

                                                /* check if there are other gigs to hide the table header and show the no gigs found */
                                                if ($('tr[data-gig-id]').length == 0) {
                                                    $('.gig-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiGigsDatatable)) {
                uiGigsDatatable.destroy();
            }

            /* gigs table initialize datatable */
            uiGigsDatatable = uiGigsTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
