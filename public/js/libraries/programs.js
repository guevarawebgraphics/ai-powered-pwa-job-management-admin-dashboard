(function () {
     ...
     *
     * */
    CPlatform.prototype.program = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiProgramTable = $('#programs-table');
            uiCreateProgramForm = $('#create-program');
            uiEditprogramForm = $('#edit-program');
            uiProgramDatatable = null;
            ...
            uiProgramDatatable = platform.region.initialize_datatable();

            /* create press validation */
            uiCreateProgramForm.find('[type="submit"]').on('click', function () {
            ...
            uiEditprogramForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            $('body').on('click', '.delete-program-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-program-id')},
                            'url': self.attr('data-program-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                if (platform.var_check(uiProgramDatatable)) {
                                                    uiProgramDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiProgramDatatable = platform.program.initialize_datatable();

                                                /* check if there are other presses to hide the table header and show the no presses found */
                                                if ($('tr[data-program-id]').length == 0) {
                                                    $('.program-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiProgramDatatable)) {
                uiProgramDatatable.destroy();
            }

            /* presses table initialize datatable */
            uiProgramDatatable = uiProgramTable.DataTable({
                "order": [],
                "paging": true,
                "pageLength": 10,
