(function () {
     ...
     *
     * */
    CPlatform.prototype.faq = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiFaqsTable = $('#faqs-table');
            uiCreateFaqForm = $('#create-faq');
            uiEditFaqForm = $('#edit-faq');
            uiFaqsDatatable = null;
            ...
            uiFaqsDatatable = platform.faq.initialize_datatable();

            /* create press validation */
            uiCreateFaqForm.find('[type="submit"]').on('click', function () {
            ...
            uiEditFaqForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                if (platform.var_check(uiFaqsDatatable)) {
                                                    uiFaqsDatatable.row(self.parents('tr:first')).remove();
                                                }
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiFaqsDatatable)) {
                uiFaqsDatatable.destroy();
            }

            /* presses table initialize datatable */
            uiFaqsDatatable = uiFaqsTable.DataTable({
                "order": [],
                "paging": true,
                "pageLength": 10,
