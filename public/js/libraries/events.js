(function () {
     ...
     *
     * */
    CPlatform.prototype.event = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiEventTable = $('#events-table');
            uiCreateEventForm = $('#create-event');
            uiEditEventorm = $('#edit-event');
            uiEventDatatable = null;
            ...
            uiEventDatatable = platform.event.initialize_datatable();

            /* create press validation */
            uiCreateEventForm.find('[type="submit"]').on('click', function () {
            ...
            uiEditEventorm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            $('body').on('click', '.delete-event-btn', function (e) {
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-event-id')},
                            'url': self.attr('data-event-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                if (platform.var_check(uiEventDatatable)) {
                                                    uiEventDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiEventDatatable = platform.event.initialize_datatable();

                                                /* check if there are other presses to hide the table header and show the no presses found */
                                                if ($('tr[data-event-id]').length == 0) {
                                                    $('.event-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiEventDatatable)) {
                uiEventDatatable.destroy();
            }

            /* presses table initialize datatable */
            uiEventDatatable = uiEventTable.DataTable({
                "order": [],
                "paging": true,
                "pageLength": 10,
