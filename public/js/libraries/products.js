(function () {
     ...
     *
     * */
    CPlatform.prototype.product = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiProductsTable = $('#products-table');
            uiCreateProductForm = $('#create-product');
            uiEditProductForm = $('#edit-product');
            uiProductsDatatable = null;
            uiInputBackgroundImage = $('input[name="image"]');
            uiRemoveImgBtn = $('.remove-image-btn');

            uiProductsDatatable = platform.product.initialize_datatable();

            /* create product validation */
            uiCreateProductForm.find('[type="submit"]').on('click', function () {
            ...
            uiEditProductForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            $('body').on('click', '.delete-product-btn', function (e) {
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-product-id')},
                            'url': self.attr('data-product-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                if (platform.var_check(uiProductsDatatable)) {
                                                    uiProductsDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiProductsDatatable = platform.product.initialize_datatable();

                                                /* check if there are other products to hide the table header and show the no products found */
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiProductsDatatable)) {
                uiProductsDatatable.destroy();
            }

            /* products table initialize datatable */
            uiProductsDatatable = uiProductsTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
