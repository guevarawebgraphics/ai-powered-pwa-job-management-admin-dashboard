(function () {
    ...
    var uiProductCategoriesTable,
        uiCreateProductCategoryForm,
        uiEditProductCategoryForm,
        uiProductCategoriesDatatable,
        filler;
    ...
    /*
     * This js file will only contain product events
     *
     * */
    CPlatform.prototype.product_category = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiProductCategoriesTable = $('#products-table');
            uiCreateProductCategoryForm = $('#create-product_category');
            uiEditProductCategoryForm = $('#edit-product');
            uiProductCategoriesDatatable = null;

            uiProductCategoriesDatatable = platform.product_category.initialize_datatable();

            /* create product validation */
            uiCreateProductCategoryForm.find('[type="submit"]').on('click', function () {
            ...
            uiCreateProductCategoryForm.validate({
            ...
            uiEditProductCategoryForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            uiEditProductCategoryForm.validate({
                errorClass: 'help-block animation-slideDown',
                errorElement: 'span',
                ignore: [':hidden:not([type="file"])'],
                errorPlacement: function (error, e) {
            ...
            /* delete product button ajax */
            $('body').on('click', '.delete-product_category-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-product_category-id')},
                            'url': self.attr('data-product_category-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                // $('[data-product-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiProductCategoriesDatatable)) {
                                                    uiProductCategoriesDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiProductCategoriesDatatable = platform.product_category.initialize_datatable();

                                                /* check if there are other product_categories to hide the table header and show the no product_categories found */
                                                if ($('tr[data-product_category-id]').length == 0) {
                                                    $('.product-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiProductCategoriesDatatable)) {
                uiProductCategoriesDatatable.destroy();
            }

            /* product_categories table initialize datatable */
            uiProductCategoriesDatatable = uiProductCategoriesTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
            ...
            return uiProductCategoriesDatatable;
        },
    }
...
$(window).on('load', function () {
    platform.product_category.initialize();
});