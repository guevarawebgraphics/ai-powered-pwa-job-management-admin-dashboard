(function () {
    ...
    var uiCouponCodesTable,
        uiCreateCouponCodeForm,
        uiEditCouponCodeForm,
        uiCouponCodesDatatable,
        uiInputImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
    ...
    /*
     * This js file will only contain coupon_code events
     *
     * */
    CPlatform.prototype.coupon_code = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiCouponCodesTable = $('#coupon_codes-table');
            uiCreateCouponCodeForm = $('#create-coupon_code');
            uiEditCouponCodeForm = $('#edit-coupon_code');
            uiCouponCodesDatatable = null;
            ...
            /* delete coupon_code button ajax */
            $('body').on('click', '.delete-coupon_code-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-coupon_code-id')},
                            'url': self.attr('data-coupon_code-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove coupon_code container */
                                                // $('[data-coupon_code-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiCouponCodesDatatable)) {
                                                    uiCouponCodesDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiCouponCodesDatatable = platform.coupon_code.initialize_datatable();

                                                /* check if there are other coupon_codes to hide the table header and show the no coupon_codes found */
                                                if ($('tr[data-coupon_code-id]').length == 0) {
                                                    $('.coupon_code-empty').removeClass('johnCena');
            ...
            /* input file event */
            uiInputImage.on('change', function () {
                var input = $(this),
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiCouponCodesDatatable)) {
                uiCouponCodesDatatable.destroy();
            }

            /* coupon_codes table initialize datatable */
            uiCouponCodesDatatable = uiCouponCodesTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
...
$(window).on('load', function () {
    platform.coupon_code.initialize();
});