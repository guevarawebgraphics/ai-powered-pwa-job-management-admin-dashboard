...
@section('content')
    <ul class="breadcrumb breadcrumb-top">
        ...
        <li><span href="javascript:void(0)">Edit DefaultTemplate</span></li>
    </ul>
    <div class="row">
        {{  Form::open([
            'method' => 'PUT',
        ...
        <div class="col-md-12">
            <div class="block">
                <div class="block-title">
                    <h2><i class="fa fa-pencil"></i> <strong>Edit DefaultTemplate "{{$template_snake_case->name}}"</strong></h2>
                </div>
                <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-9">
                        <input type="text" class="form-control" id="template_snake_case_name" name="name"
                               value="{{  Request::old('name') ? : $template_snake_case->name }}"
                               placeholder="Enter DefaultTemplate name..">
                        @if($errors->has('name'))
                ...
                <div class="form-group{{ $errors->has('slug') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-9">
                        <input type="text" class="form-control" id="template_snake_case_slug" name="slug"
                               value="{{  Request::old('slug') ? : $template_snake_case->slug }}"
                               placeholder="Enter DefaultTemplate slug..">
                        @if($errors->has('slug'))
                ...
                <div class="form-group{{ $errors->has('banner_image') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-offset-3 col-md-9">
                        <a href="{{ asset($template_snake_case->banner_image) }}" class="zoom img-thumbnail" style="cursor: default !important;" data-toggle="lightbox-image">
                            <img src="{{ $template_snake_case->banner_image != '' ? asset($template_snake_case->banner_image) : '' }}"
                                 alt="{{ $template_snake_case->banner_image != '' ? asset($template_snake_case->banner_image) : '' }}"
                        ...
                        <a href="javascript:void(0)" class="btn btn-xs btn-danger remove-image-btn"
                           style="display: {{ $template_snake_case->banner_image != '' ? '' : 'none' }};"><i class="fa fa-trash"></i> Remove</a>
                ...
                <div class="form-group file-container {{ $errors->has('file') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-offset-3 col-md-9">
                        <a target="_blank" href="{{ asset($template_snake_case->file) }}" class="file-anchor">
                            {{ $template_snake_case->file }}
                        </a>
                        <br>
                        <a href="javascript:void(0)" class="btn btn-xs btn-danger remove-file-btn"
                           style="display: {{ $template_snake_case->file != '' ? '' : 'none' }};"><i class="fa fa-trash"></i> Remove</a>
                ...
                <div class="form-group{{ $errors->has('content') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-9">
                    <textarea id="template_snake_case_content" name="content" rows="9" class="form-control ckeditor"
                              placeholder="Enter DefaultTemplate content..">{!! Request::old('content') ? : $template_snake_case->content !!}</textarea>
                    ...
                    <div class="col-md-9">
                        <label class="switch switch-primary">
                            <input type="checkbox" id="is_active" name="is_active"
                                   value="1" {{ Request::old('is_active') ? : ($template_snake_case->is_active ? 'checked' : '') }}>
                            <span></span>
                        </label>
                    </div>
                </div>
