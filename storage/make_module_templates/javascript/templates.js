(function () {
    ...
    var uiTemplateCamelCasePluralTable,
        uiCreateTemplateCamelCaseForm,
        uiEditTemplateCamelCaseForm,
        uiTemplateCamelCasePluralDatatable,
        uiInputBannerImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
    ...
    /*
     * This js file will only contain template_snake_case events
     *
     * */
    CPlatform.prototype.template_snake_case = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiTemplateCamelCasePluralTable = $('#template_snake_case_plural-table');
            uiCreateTemplateCamelCaseForm = $('#create-template_snake_case');
            uiEditTemplateCamelCaseForm = $('#edit-template_snake_case');
            uiTemplateCamelCasePluralDatatable = null;
            ...
            uiTemplateCamelCasePluralDatatable = platform.template_snake_case.initialize_datatable();

            /* create template_snake_case validation */
            uiCreateTemplateCamelCaseForm.find('[type="submit"]').on('click', function () {
            ...
            uiCreateTemplateCamelCaseForm.validate({
            ...
            /* edit template_snake_case validation */
            uiEditTemplateCamelCaseForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            uiEditTemplateCamelCaseForm.validate({
                errorClass: 'help-block animation-slideDown',
                errorElement: 'span',
                ignore: [':hidden:not([type="file"])'],
                errorPlacement: function (error, e) {
            ...
            /* delete template_snake_case button ajax */
            $('body').on('click', '.delete-template_snake_case-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-template_snake_case-id')},
                            'url': self.attr('data-template_snake_case-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove template_snake_case container */
                                                // $('[data-template_snake_case-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiTemplateCamelCasePluralDatatable)) {
                                                    uiTemplateCamelCasePluralDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiTemplateCamelCasePluralDatatable = platform.template_snake_case.initialize_datatable();

                                                /* check if there are other template_snake_case_plural to hide the table header and show the no template_snake_case_plural found */
                                                if ($('tr[data-template_snake_case-id]').length == 0) {
                                                    $('.template_snake_case-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiTemplateCamelCasePluralDatatable)) {
                uiTemplateCamelCasePluralDatatable.destroy();
            }

            /* template_snake_case_plural table initialize datatable */
            uiTemplateCamelCasePluralDatatable = uiTemplateCamelCasePluralTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
            ...
            return uiTemplateCamelCasePluralDatatable;
        },
    }
...
$(window).on('load', function () {
    platform.template_snake_case.initialize();
});